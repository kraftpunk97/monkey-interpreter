...
//
// Created by kxg220013 on 12/15/2023.
//
...
expr_t* expr_prefix_create(token_t token, char* op, expr_t* value) {
    expr_t* expr = (expr_t* ) malloc(sizeof(expr_t));
    if (expr == NULL) {
...
expr_t* expr_infix_create(token_t token, char* op, expr_t* left_expr, expr_t* right_expr) {
    expr_t* expr = (expr_t* ) malloc(sizeof(expr_t));
    if (expr == NULL) {
        fprintf(stderr, "ERROR in 'expr_prefix_create': Failed to allocate 'expr'!\n");
        exit(EXIT_FAILURE);
...
expr_t* expr_fn_defn_create(token_t token, list parameters, stmt_t* body) {
    expr_t* expr = (expr_t* ) malloc(sizeof(expr_t));
    if (expr == NULL) {
